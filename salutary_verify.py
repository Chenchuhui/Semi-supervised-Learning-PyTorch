...
def main():
    ...
    if args.resume:
        ...
        if os.path.exists(f"{args.out}/incorrect_labeled_indices.txt"):
            with open(f"{args.out}/incorrect_labeled_indices.txt", "r") as f:
                labels = []
                for line in f:
...
def updated_ulb_dataset(args, epoch):
    ...
    # # else:
    with open(f"./influence_scores_seed{args.seed}/influence_scores_{epoch}.txt", "r") as f:
        lines = f.readlines()
    ...
    while True:
        filtered_indices = [ulb_indices[i] for i in range(len(ulb_indices)) if influence_scores[i] < influence_threshold]
        filtered_salutary_labels = [salutary_labels[i] for i in range(len(ulb_indices)) if influence_scores[i] < influence_threshold] 
        if filtered_indices:
            break
    ...
    return torch.tensor(filtered_indices), torch.tensor(filtered_salutary_labels)
...
def train(args, labeled_trainloader, unlabeled_dataset, val_dataset, test_loader,
    ...
    for epoch in range(args.start_epoch, epochs):
        ...
        if not warmup:
            ...
            positive_indices, salutary_labels = updated_ulb_dataset(args, epoch)
            unlabeled_labeled_indices.extend(positive_indices.tolist())
            ...
            unlabeled_dataset.targets[positive_indices] = salutary_labels
        ...
        #     # Get the indices with positive influence
        #     lb_indices, ulb_indices = updated_ulb_dataset(expanded_dataset, val_dataset, model_copy, args, epoch, indices)
            
        #     if len(lb_indices) != 0:
        #         selected_indices = indices[lb_indices // args.num_classes]
        ...
        #         unlabeled_dataset.targets[selected_indices] = expanded_labels[lb_indices]
        ...
        #     # Expand the labeled dataset with the selected indices
        #     updated_lb_dataset = copy.deepcopy(labeled_dataset)
        #     salutary_labeled_dataset = unlabeled_dataset.select_subset(unlabeled_labeled_indices)
        #     updated_lb_dataset.data = np.concatenate((salutary_labeled_dataset.data, updated_lb_dataset.data), axis=0)
        #     updated_lb_dataset.targets = np.concatenate((salutary_labeled_dataset.targets, updated_lb_dataset.targets), axis=0)
        #     updated_lb_dataset.gt = np.concatenate((salutary_labeled_dataset.gt, updated_lb_dataset.gt), axis=0)

        #     # unlabeled_dataset is from ulb_indices
        #     ulb_dataset = unlabeled_dataset.select_subset(ulb_indices)
        #     if isinstance(ulb_dataset.targets, list):
        #             ulb_dataset.targets = torch.tensor(ulb_dataset.targets)
        #     ulb_dataset.targets = expanded_labels[ulb_indices]

        #     print(f"Unlabeled dataset size: {len(ulb_dataset)}")
        #     # unlabeled_ratio = len(ulb_dataset) / len(unlabeled_dataset)

        #     ulb_expand_factor = args.batch_size * args.mu * args.train_iteration // len(ulb_dataset)
        #     lb_expand_factor = args.batch_size * args.mu * args.train_iteration // len(updated_lb_dataset)
        #     ulb_dataset.expand_data(ulb_expand_factor)
        #     updated_lb_dataset.expand_data(lb_expand_factor)

        #     labeled_trainloader = DataLoader(updated_lb_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, drop_last=True)
