...
class CTAugmentTransform:
    """
    A PyTorch-compatible wrapper for CTAugment.
    """
    def __init__(self, cta: CTAugment, probe=False):
        """
        Args:
            cta: An instance of the CTAugment class.
            probe: If True, sample augmentations without learning.
        """
        self.cta = cta
        self.probe = probe
    ...
    def __call__(self, img):
        """
        Apply CTAugment to the input image.
        Args:
            img: Input image as a PIL Image or numpy array.
        Returns:
            Augmented image as a Tensor.
        """
        if isinstance(img, torch.Tensor):
            img = transforms.ToPILImage()(img)

        # Apply CTAugment
        policy = self.cta.policy(self.probe)
        img = apply(np.array(img), policy)
